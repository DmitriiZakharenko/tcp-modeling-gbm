...
Clinical and imaging metadata were downloaded from TCIA (Version 3, DOI: [10.7937/v9pn-2f72](https://doi.org/10.7937/v9pn-2f72)) [8]. Inclusion required t0 RTDOSE and GTV NIfTI, known fractionation dose and fraction number. One patient was excluded after DVH QC (Dmean=0 Gy), yielding **n=190** for modeling (120×60 Gy/30 fr; 61×40.05 Gy/15 fr) [2,15].
...
- PyRadiomics comparison uses in-sample feature selection (top-5 by univariate AUC) [17]; nested CV for feature selection would further reduce optimism [7,18]
...
1. Title & motivation (TCP + open data) [4,8]
2. CFB-GBM overview (TCIA v3, RANO) [8–10]
3. Cohort flowchart (264→190)
4. Methods pipeline diagram
5. TCP model equations (1 slide) [4–6,11]
6. Clinical results: OS by scheme + WHO PS [2,15]
7. Pooled TCP OS proxy (AUC 0.68) + caveat [7]
8. RANO pooled failure (AUC 0.43) — confounding [12,15]
9. Pooled volume+scheme models (AUC 0.72) + LOOCV 0.64
10. Within-arm 40 Gy: volume→RANO + LOOCV 0.74 [16]
11. PyRadiomics vs DVH volume (AUC 0.78 vs 0.71) [9,17]
12. Cox: RANO predicts OS [14]
13. Limitations (honest)
14. Comparison to Moreau radiomics + checklist [9,10]
15. Conclusions + DOI [8]

---

*Numbers verified against `reports/metrics/` on 2026-06-28. Reference DOIs verified in `reports/literature_table.csv`.*
